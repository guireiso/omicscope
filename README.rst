...
Once obtained differential expression data, user can perform Over-Representation Analysis (ORA) or Gene-Set Enrichment Analysis (GSEA), which were implemented  according to `GSEApy <https://gseapy.readthedocs.io/>`__ in EnrichmentScope module.
...
Finally, for each experiment performed by *OmicScope*, it is possible to export an omics file (.omics extension) for further use in **Nebula workflow**. Nebula is a specialized module that unables analysis of multiple studies or comparisons, providing several types of analysis to compare those groups and find similarities among them.
